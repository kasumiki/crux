...
Crux supports user-defined processors loaded from `~/.crux/processors/`.

## Quick Start

1. Create a Python file with a class that inherits from `crux.processors.base.Processor`
2. Implement `can_handle()`, `process()`, `name`, and set `priority`
3. Copy the file to `~/.crux/processors/` (or run `crux init-processor <name>` to scaffold one)

```bash
# Install the example ansible processor
cp examples/custom_processor/ansible_output.py ~/.crux/processors/
```
...
By default, user processors are loaded from `~/.crux/processors/`. To change this, set `user_processors_dir` in your config:
...
- Enable `CRUX_DEBUG=true` to see skip messages in stderr.
