"""Tests for the crux CLI subcommands."""
...
def _run_cli(*args, stdin=None):
    ...
    result = subprocess.run(  # noqa: S603
        [sys.executable, "-m", "crux.cli", *args],
        capture_output=True,
        text=True,
        cwd=PLUGIN_DIR,
        input=stdin,
...
class TestVersionCommand:
    def test_prints_version(self):
        ...
        assert rc == 0
        assert f"crux v{__version__}" in stdout

    def test_version_format(self):
        rc, stdout, _ = _run_cli("version")
        assert rc == 0
        # Should match pattern: crux vX.Y.Z
        line = stdout.strip()
        assert line.startswith("crux v")
        version_str = line.split("crux v")[1]
...
class TestNoCommand:
    def test_no_args_shows_help(self):
        ...
        assert rc == 0
        assert "crux" in stdout.lower() or "usage" in stdout.lower()
...
class TestBenchmarkCommand:
    ...
    def test_benchmark_real_command(self):
        ...
        assert rc == 0
        assert "Crux Benchmark" in stdout
...
class TestDiffstat:
    def test_summarize_removed_lines(self):
        from crux.diffstat import summarize
    ...
    def test_format_summary_contains_sections(self):
        from crux.diffstat import format_summary, summarize
...
class TestMarketplaceDetection:
    def test_cache_path_is_marketplace_managed(self):
        from crux.cli import _is_marketplace_managed
    ...
    def test_regular_repo_not_marketplace_managed(self):
        ...
        assert _is_marketplace_managed("/Users/someone/Desktop/crux") is False

    def test_old_plugin_dir_not_marketplace_managed(self):
        from crux.cli import _is_marketplace_managed

        # Pre-marketplace layout (~/.claude/plugins/crux) is self-updatable.
        path = os.path.join(os.path.expanduser("~"), ".claude", "plugins", "crux")
        assert _is_marketplace_managed(path) is False


class TestBinScript:
    def test_bin_script_exists_and_executable(self):
        bin_path = os.path.join(PLUGIN_DIR, "bin", "crux")
    ...
    def test_bin_script_runs_version(self):
        ...
        assert f"crux v{__version__}" in result.stdout
