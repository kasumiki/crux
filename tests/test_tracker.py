...
class TestSavingsTracker:
    ...
    def test_session_id_from_env(self):
        """CRUX_SESSION env var should set the session ID."""
        os.environ["CRUX_SESSION"] = "env-session-42"
        try:
            tracker = SavingsTracker()
            assert tracker.session_id == "env-session-42"
            tracker.close()
        finally:
            del os.environ["CRUX_SESSION"]
    ...
    def test_fallback_session_id_is_stable_across_instances(self):
        ...
        """
        os.environ.pop("CRUX_SESSION", None)
        t1 = SavingsTracker()
        t2 = SavingsTracker()
        try:
...
class TestStatsCLI:
    ...
    def setup_method(self):
        ...
        # Use savings.db to match what stats.py creates via CRUX_DB_DIR
        SavingsTracker.DB_DIR = self.tmp_dir
    ...
    def test_empty_db_human(self):
        ...
        assert "Crux Savings" in result.stdout
