...
from crux.engine import CompressionEngine

# --- Debug logging (writes to data_dir/hook.log when CRUX_DEBUG=true) ---
_log = logging.getLogger("crux.wrap")
_log.setLevel(logging.DEBUG)
_debug = os.environ.get("CRUX_DEBUG", "").lower() in (
    "1",
    "true",
    "yes",
)
...
# Audit logging + savings/mismatch recording live in crux.core, shared with the
    ...
    try:
        ...
        if child_proc:
            ...
            try:
                ...
                partial_out, partial_err = "", ""
        note = f"[crux] Command timed out after {timeout}s (partial output shown)"
        ...
        print(f"[crux] Failed to execute: {e}", file=sys.stderr)
        sys.exit(127)
    finally:
...
def _cap_output(output: str) -> str:
    ...
    if not cap or cap <= 0 or len(output) <= cap:
        return output
    note = f"\n[crux] Output truncated at {cap:,} chars (was {len(output):,})\n"
...
def _print_dry_run(
    ...
    print(
        f"[crux dry-run] processor={processor_name} "
...
def main():
    ...
    if is_chain:
        ...
        # Final stage: enforce the token budget across the whole chain output.
        budgeted = core.apply_budget(compressed)
