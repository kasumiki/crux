"""Check for new Crux releases via GitHub API."""
...
_GITHUB_API_URL = "https://api.github.com/repos/kasumiki/crux/releases/latest"
...
def _fetch_latest_version(fetch_fn=None, timeout=1):
    ...
    req = urllib.request.Request(  # noqa: S310
        _GITHUB_API_URL,
        headers={"Accept": "application/vnd.github.v3+json", "User-Agent": "crux"},
    )
...
def check_for_update(fetch_fn=None, cache_ttl=_CACHE_TTL_SECONDS):
    """Check if a newer version of Crux is available.
    ...
    try:
        ...
        if _parse_version(latest) > _parse_version(__version__):
            return f"Update available: v{__version__} -> v{latest} -- Run: crux update"
    except Exception:
        return None
