...
```bash
git remote add upstream https://github.com/<org>/crux.git
git fetch upstream
...
```
crux/                       (repo root)
├── plugin/                 # the self-contained Claude + Antigravity plugin
│   ├── .claude-plugin/     #   plugin.json
│   ├── crux/               #   the Python package (engine, gate, processors, …)
│   ├── scripts/            #   Claude hook entry shims
│   ├── antigravity/        #   Antigravity hook
│   ├── hooks/ commands/ skills/ bin/
│   └── CLAUDE.md
├── .claude-plugin/         # marketplace.json (catalog; points source at ./plugin)
├── installers/ install.py  # install/uninstall machinery
├── tools/                  # dev scripts (audit, demo/table generator)
├── examples/ tests/        # runnable demo + fixtures, and the test suite
└── pyproject.toml  README.md  …
```

The `plugin/` directory is what ships to users — the marketplace copies only that
subtree. Everything else (installers, tools, examples, tests) is repo-only. The CLI/tests
import the package as `crux` (resolved from `plugin/` via `pyproject.toml` `pythonpath`).

## Adding a new processor

Adding a new processor is a **single-file operation**. Create one file in
`plugin/crux/processors/` and it will be automatically discovered.

### Quick start

1. Create `plugin/crux/processors/my_tool.py`
2. Run `python3 -m pytest tests/ -v` to verify

That's it. No other files need editing.

> **Changing compression behavior?** The savings table and the "See it in action"
> example in `README.md` are generated from the fixtures in `examples/fixtures/` by the
> real pipeline. If you change a processor, the reducer, or a fixture, regenerate them with
> `python3 tools/generate_demo.py` — `tests/test_readme_claims.py` fails until the README
> matches reality, so the docs can never over-claim.
...
```python
from crux.processors.my_tool import MyToolProcessor
```
